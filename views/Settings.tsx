...
import { AppSettings, Staff, PrintSettings, WhatsAppTemplates, StaffRole, TCTemplate, MyInvoisSettings, LalamoveSettings, PaymentGatewayConfig } from '../types';
import { 
  ...
  Key, Cpu, Link as LinkIcon, Loader2, Navigation
} from 'lucide-react';
...
export const Settings: React.FC<SettingsProps> = ({ settings, setSettings, staff, setStaff, onBackup, onRestore, onReset, currentUser, onLogout }) => {
  const [activeTab, setActiveTab] = useState<'General' | 'Finance' | 'Lists' | 'Templates' | 'Printing' | 'Notifications' | 'Staff' | 'My Profile' | 'E-Invoice' | 'Backup & Restore' | 'Payments' | 'API Config'>('General');
  ...
  const [isSavingLalamove, setIsSavingLalamove] = useState(false);
  const [lalamoveSaveStatus, setLalamoveSaveStatus] = useState<'idle' | 'success' | 'error'>('idle');

  const handleChange = (key: keyof AppSettings, value: any) => {
     setSettings(prev => ({ ...prev, [key]: value }));
  };

  const handleGoogleDriveChange = (key: string, value: any) => {
    setSettings(prev => ({
      ...prev,
      googleDrive: { ...(prev.googleDrive || { isConnected: false, autoBackup: false }), [key]: value }
    }));
    setApiSaveStatus('idle');
  };

  const handleLalamoveChange = (key: string, value: any) => {
    setSettings(prev => ({
      ...prev,
      lalamove: { ...(prev.lalamove || { apiKey: '', apiSecret: '', environment: 'Sandbox', isConnected: false }), [key]: value }
    }));
    setLalamoveSaveStatus('idle');
  };

  const handleSaveGDriveAPI = async () => {
    if (!settings.googleDrive?.clientId || !settings.googleDrive?.apiKey) {
      alert("Sila masukkan Client ID dan API Key.");
      return;
    }
    ...
    try {
      ...
      initializeGoogleDrive(
        ...
        settings.googleDrive.apiKey, 
        (success) => {
          setIsSavingAPI(false);
          if (success) {
            setApiSaveStatus('success');
            handleGoogleDriveChange('isConnected', true);
            setTimeout(() => setApiSaveStatus('idle'), 3000);
          } else {
            setApiSaveStatus('error');
            alert("Gagal menyambung ke Google Drive. Sila pastikan kredensial anda betul.");
          }
        }
      );
    } catch (e) {
  ...
  const handleSaveLalamoveAPI = async () => {
    if (!settings.lalamove?.apiKey || !settings.lalamove?.apiSecret) {
      alert("Sila masukkan API Key dan API Secret Lalamove.");
      return;
    }

    setIsSavingLalamove(true);
    setLalamoveSaveStatus('idle');

    try {
      // Simulate API validation
      await new Promise(resolve => setTimeout(resolve, 1500));
      setIsSavingLalamove(false);
      setLalamoveSaveStatus('success');
      handleLalamoveChange('isConnected', true);
      setTimeout(() => setLalamoveSaveStatus('idle'), 3000);
    } catch (e) {
      setIsSavingLalamove(false);
      setLalamoveSaveStatus('error');
    }
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 min-h-[600px] overflow-hidden">
         ...
         {activeTab === 'API Config' && (
           <div className="p-8 space-y-10 animate-fadeIn overflow-y-auto max-h-[80vh] scrollbar-thin">
              <div className="bg-slate-900 text-white p-8 rounded-3xl shadow-xl flex items-center gap-6 relative overflow-hidden shrink-0">
                  <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10"></div>
                  <div className="p-4 bg-white/10 rounded-2xl backdrop-blur-md relative z-10"><Key size={32}/></div>
                  <div className="relative z-10">
                     <h3 className="text-2xl font-black italic">Pusat Integrasi API</h3>
                     <p className="text-slate-400">Konfigurasi kunci akses pihak ketiga dan status sistem AI.</p>
                  </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 ...
                 <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 space-y-6">
                    <div className="flex justify-between items-center">
                       <h4 className="text-lg font-black text-slate-800 uppercase tracking-tight flex items-center gap-2">
                          <Cloud size={20} className="text-blue-500"/> Google Cloud Storage
                       </h4>
                       <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${settings.googleDrive?.isConnected ? 'bg-green-100 text-green-700' : 'bg-slate-200 text-slate-500'}`}>
                          {settings.googleDrive?.isConnected ? 'Connected' : 'Offline'}
                       </span>
                    </div>

                    <div className="space-y-4">
                       <div>
                          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Google Client ID</label>
                          <input 
                            type="text" 
                            className="w-full border-2 border-white p-3 rounded-xl bg-white shadow-sm focus:border-blue-500 outline-none font-mono text-xs" 
                       ...
                       <div>
                          <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Google API Key</label>
                          <input 
                            type="password" 
                            title="API Key"
                            className="w-full border-2 border-white p-3 rounded-xl bg-white shadow-sm focus:border-blue-500 outline-none font-mono text-xs" 
                       ...
                       <div className="bg-blue-50 p-4 rounded-xl border border-blue-100">
                          <p className="text-[10px] text-blue-700 font-bold leading-relaxed">
                             ...
                             Gunakan kredensial ini untuk membolehkan sistem melakukan backup automatik ke Google Drive anda. Aktifkan **Google Drive API** di console.cloud.google.com.
                          </p>
                       </div>

                       <div className="pt-2">
                          <button 
                            onClick={handleSaveGDriveAPI}
                            disabled={isSavingAPI}
                            className={`w-full py-3 rounded-xl font-black transition-all flex items-center justify-center gap-2 shadow-lg ${
                 ...
                 <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 space-y-6">
                    <div className="flex justify-between items-center">
                       <h4 className="text-lg font-black text-slate-800 uppercase tracking-tight flex items-center gap-2">
                          <Navigation size={20} className="text-orange-500"/> Lalamove Delivery
                       </h4>
                       <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${settings.lalamove?.isConnected ? 'bg-orange-100 text-orange-700' : 'bg-slate-200 text-slate-500'}`}>
                          {settings.lalamove?.isConnected ? 'Active' : 'Not Connected'}
                       </span>
                    </div>
                    ...
                    <div className="space-y-4">
                       <div>
                          ...
                          <input 
                            type="text" 
                            className="w-full border-2 border-white p-3 rounded-xl bg-white shadow-sm focus:border-orange-500 outline-none font-mono text-xs" 
                            value={settings.lalamove?.apiKey || ''} 
                            onChange={e => handleLalamoveChange('apiKey', e.target.value)}
                            placeholder="Lalamove API Key"
                          />
                       </div>
                       <div>
                          ...
                          <input 
                            ...
                            title="API Secret"
                            className="w-full border-2 border-white p-3 rounded-xl bg-white shadow-sm focus:border-orange-500 outline-none font-mono text-xs" 
                            value={settings.lalamove?.apiSecret || ''} 
                            onChange={e => handleLalamoveChange('apiSecret', e.target.value)}
                            placeholder="••••••••••••••••"
                          />
                       </div>
                       <div>
                          ...
                          <div className="bg-white p-1 rounded-xl border-2 border-slate-100 flex">
                              <button onClick={() => handleLalamoveChange('environment', 'Sandbox')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${settings.lalamove?.environment === 'Sandbox' ? 'bg-orange-100 text-orange-700 shadow-sm' : 'text-slate-400 hover:bg-slate-50'}`}>Sandbox</button>
                              <button onClick={() => handleLalamoveChange('environment', 'Production')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${settings.lalamove?.environment === 'Production' ? 'bg-orange-600 text-white shadow-sm' : 'text-slate-400 hover:bg-slate-50'}`}>Production</button>
                          </div>
                       </div>
                       
                       <div className="pt-2">
                          <button 
                            onClick={handleSaveLalamoveAPI}
                            disabled={isSavingLalamove}
                            className={`w-full py-3 rounded-xl font-black transition-all flex items-center justify-center gap-2 shadow-lg ${
                              lalamoveSaveStatus === 'success' 
                              ? 'bg-green-600 text-white' 
                              : lalamoveSaveStatus === 'error'
                              ? 'bg-red-600 text-white'
                 ...
                 {/* AI Core Status (Read Only as per requirements) */}
                 <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 space-y-6">
                    <h4 className="text-lg font-black text-slate-800 uppercase tracking-tight flex items-center gap-2">
                       <Cpu size={20} className="text-purple-500"/> AI Core Engine
                    </h4>

                    <div className="bg-white p-6 rounded-2xl border-2 border-slate-100 shadow-sm space-y-4">
                       <div className="flex items-center justify-between">
                          <span className="text-sm font-bold text-slate-600">Model Status</span>
                          <div className="flex items-center gap-2 text-green-600 font-black text-xs uppercase tracking-widest">
                             <CheckCircle size={16}/> Active (Gemini 3 Pro)
                          </div>
                       </div>
                       <div className="flex items-center justify-between">
                          <span className="text-sm font-bold text-slate-600">Integration</span>
                          <span className="text-blue-600 font-black text-xs uppercase tracking-widest">System Managed</span>
                       </div>
                       <div className="pt-4 border-t border-slate-50">
                          <p className="text-[10px] text-slate-400 font-medium italic">
                             * Konfigurasi kunci AI utama diuruskan oleh pelayan pusat untuk memastikan kestabilan diagnostik dan prestasi penjanaan imej. Pengguna tidak perlu memasukkan kunci AI secara manual.
                          </p>
                       </div>
                    </div>
                    ...
                    <div className="p-4 bg-purple-50 rounded-xl border border-purple-100 flex items-center gap-3">
                       ...
                       <p className="text-[10px] text-purple-700 font-bold">
                          AI Diagnostic Engine sedia untuk memproses data pembaikan dan ringkasan teknikal.
                       </p>
                    </div>
                 </div>
              </div>
