...
export default function App() {
  ...
  // Sync state to local and remote storage - Removed .length checks to allow persisting empty states (after deletion)
  useEffect(() => { if (!isLoading) { saveStoreData('jobs', jobs); if (jobs.length > 0) syncItem('repair_jobs', jobs[0].id, jobs[0]); } }, [jobs, isLoading]);
  useEffect(() => { if (!isLoading) { saveStoreData('inventory', inventory); if (inventory.length > 0) inventory.slice(0, 5).forEach(item => syncItem('inventory', item.id, item)); } }, [inventory, isLoading]);
  useEffect(() => { if (!isLoading) { saveStoreData('customers', customers); if (customers.length > 0) syncItem('customers', customers[0].id, customers[0]); } }, [customers, isLoading]);
  useEffect(() => { if (!isLoading) { saveStoreData('sales', sales); if (sales.length > 0) syncItem('sales', sales[sales.length - 1].id, sales[sales.length - 1]); } }, [sales, isLoading]);
